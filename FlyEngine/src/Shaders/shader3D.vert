...
void main()
{
	if(animation_State==1)
	{
	 ...
	 FragPos = vec3(model *bone_transform* vec4(aPos,1.0));
	// light_vector = light_position - (model *bone_transform* vec4(aPos, 1.0));
	  
	  vec4 Normal_temp = bone_transform * vec4(aNormal, 0.0);
	
	Normal = mat3(transpose(inverse(model))) * Normal_temp.xyz;  
	 vshadow_position =	shadowMatrix  * model *bone_transform* vec4(aPos, 1.0);
